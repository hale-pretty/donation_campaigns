...
import { CampaignEditBox, EditBoxHeader, EditBoxBody, EditBoxBodySection, ImageFilesCtn, ImageFile, PerkCtn, Perk, EmptyPerk, EmptyImage } from "./styles";
...
function NewCampaign({ setIsAdding }) {
    const { id, token } = useSelector(state => state.user.info);
    ...
    const handleSubmitCampaign = async (e) => {
        ...
        const res = await graphql_api.addNewCampaign({
            input: inputs,
            images: imagesFile,
            perks: perks,
        }, token);
    ...
    return (
        <CampaignEditBox>
            <EditBoxHeader>
                <h1>{title === "" ? "New Campaign Title" : title}</h1>
            ...
            <EditBoxBody>
                ...
                <EditBoxBodySection>
                    ...
                    <ImageFilesCtn>
                        ...
                        {(imagesFile.length > 0) && imagesFile.map((img, index) => (
                            <ImageFile key={index}>
                                <img src={img} />
                            </ImageFile>
                        ))}
